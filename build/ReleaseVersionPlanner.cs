...
public static class ReleaseVersionPlanner
{
    ...
    private static readonly Regex ReleaseMarkerPattern = new(@"\[release:\s*(?<value>[^\]\r\n]+)\]",
                                                             RegexOptions.Compiled
                                                           | RegexOptions.CultureInvariant
                                                           | RegexOptions.IgnoreCase);

    public static ReleaseVersionPlan Plan(IEnumerable<string> existingTags,
                                          string commitMessage)
    {
        var tagSet = existingTags.Select(x => x.Trim())
                                 .Where(x => !string.IsNullOrWhiteSpace(x))
                                 .ToHashSet(StringComparer.Ordinal);

        StableVersion latest = tagSet.Select(TryParseStableTag)
        ...
        if (string.Equals(requested,
            ...
            return new(false,
                       null,
                       null,
                       "Release tag skipped by [release: skip].");
        ...
        string version = requested.ToLowerInvariant() switch
        {
            ...
            _ => throw new InvalidOperationException(
                     $"Unsupported release override '{requested}'. Use major, minor, patch, skip, or an explicit semantic version.")
        };
    ...
    private static string GetRequestedRelease(string commitMessage)
    {
        ...
        return matches.Count == 0 ? "patch" : matches[^1].Groups["value"].Value.Trim();
    }
    ...
    private static StableVersion? TryParseStableTag(string tag)
    {
        ...
        return new(int.Parse(match.Groups["major"].Value),
    ...
    {
        public StableVersion NextMajor()
            => new(Major + 1,
                   0,
                   0);

        public StableVersion NextMinor()
            => new(Major,
                   Minor + 1,
                   0);

        public StableVersion NextPatch()
            => new(Major,
                   Minor,
                   Patch + 1);
